...
object JSONSerialization {
  ...
  given stringConverter: JSONConverter[String] with
    override def convert(value: String): JSONString = JSONString(value)

  given intConverter: JSONConverter[Int] with
    override def convert(value: Int): JSONNumber = JSONNumber(value)

  given dateConverter: JSONConverter[Date] with
    override def convert(value: Date): JSONString = JSONString(value.toString)

  given userConverter: JSONConverter[User] with
    override def convert(user: User): JSONObject = JSONObject(
      Map(
  ...
  given feedConverter: JSONConverter[Feed] with
    override def convert(feed: Feed): JSONObject = JSONObject(
      Map(
